...
// Followers lists the accepted followers of the viewer.
func (h *Handlers) Followers(c echo.Context) error {
	viewer, err := reqctx.RequireUser(c)
	if err != nil {
		return err
	}
	ctx := c.Request().Context()
	rows, err := h.Q.ListFollowers(ctx, viewer.ID, dto.Limit(c))
	if err != nil {
		return err
	}
	items := make([]api.UserRef, 0, len(rows))
	for _, row := range rows {
		items = append(items, api.UserRef{
			Username:    row.Username,
			DisplayName: row.DisplayName,
			AvatarURL:   dto.AvatarURL(row.AvatarMediaID),
		})
	}
	return c.JSON(http.StatusOK, api.UserPage{Items: items})
}

// Following lists the accounts the viewer follows.
func (h *Handlers) Following(c echo.Context) error {
	viewer, err := reqctx.RequireUser(c)
	if err != nil {
		return err
	}
	ctx := c.Request().Context()
	rows, err := h.Q.ListFollowing(ctx, viewer.ID, dto.Limit(c))
	if err != nil {
		return err
	}
	items := make([]api.UserRef, 0, len(rows))
...
// FollowRequests lists the pending incoming requests of the viewer.
func (h *Handlers) FollowRequests(c echo.Context) error {
	viewer, err := reqctx.RequireUser(c)
	if err != nil {
		return err
	}
	ctx := c.Request().Context()
	rows, err := h.Q.ListPendingFollowRequests(ctx, viewer.ID, dto.Limit(c))
	if err != nil {
		return err
	}
	items := make([]api.UserRef, 0, len(rows))
